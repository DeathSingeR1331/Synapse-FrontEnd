...
import {
    ...
    FiClock, FiZap, FiShield, FiToggleLeft, FiToggleRight
} from 'react-icons/fi';
...
const Dashboard = () => {
    ...
    // ✅ ADDED: State for the personalization engine toggle
    ...
    const handleSend = async (messageText = chatInput) => {
        ...
        try {
            // ✅ MODIFIED: Add the personalization state to the API payload
            const response = await apiCall(`/api/v1/conversations/${tempActiveChatId}/messages`, {
                method: 'POST',
                body: JSON.stringify({ 
                    content: content,
                    is_personalization_enabled: isPersonalizationEnabled // Send the toggle state
                }),
            });
        ...
        <div className="flex h-screen bg-black text-white overflow-hidden">
            ...
            >
                ...
                <div className="p-6">
                    ...
                    {/* ✅ ADDED: Personalization Engine Toggle */}
                    <AnimatePresence>
                        {isSidebarExpanded && (
                            <motion.div
                                className="mt-4"
                            ...
                            >
                                <motion.button
                                    ...
                                    className={`w-full flex items-center gap-3 text-sm py-3 px-3 rounded-lg transition-colors duration-300 ${isPersonalizationEnabled 
                                        ? 'bg-gradient-to-r from-green-600/20 to-green-700/20 text-green-400 hover:bg-green-600/30'
                                        : 'bg-gray-700/20 text-gray-400 hover:bg-gray-600/30'
                                    }`}
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                >
                                    {isPersonalizationEnabled ? (
                                        <FiToggleRight size={20} className="text-green-400" />
                                    ) : (
